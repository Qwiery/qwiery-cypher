 ...
 */
export   function toCypher(q, variable = "n", level = 0) {
	let s = "";
	if (q.operator) {
		if (q.field) {
			...
			const conjunctor = getCypherOperator(q.operator);
			const coll:string[] = [];
			for (const part of q.parts) {
				coll.push(toCypher(part, variable));
			}
